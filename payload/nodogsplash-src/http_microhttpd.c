							...
							t_client *client)
{
	...
	if (check_authdir_match(url, config->authdir)) {
		...
		// Look up email, phonenumber, firstname and the SMS-marketing opt-in.
		// sms_on is only present when the (optional) marketing checkbox was ticked;
		...
		if (email != NULL) {
			debug(LOG_DEBUG, "Email captured: %s", email);
		}
		if (firstname != NULL) {
